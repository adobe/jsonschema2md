...
function requiredProperties(properties, required) {
  ...
  return _.mapValues(properties, prop => {
  ...
  if (_.keys(schema.properties).length > 0) {
    ...
    for (let i=0; i<_.keys(schema.properties).length;i++) {
      multi.push( [ 'property.ejs', {
        ...
        examples: stringifyExamples(schema.properties[_.keys(schema.properties)[i]]['meta:example']),
        schema: schema.properties[_.keys(schema.properties)[i]] } ]);
    }
  }
