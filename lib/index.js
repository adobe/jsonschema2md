...
export function jsonschema2md(schema, options) {
  ...
  const schemas = singleFile
    ? allSchemas.filter((item) => !item[s.parent])
    : allSchemas;
...
export async function main(args) {
  ...
  const schemaFiles = await readdirpPromise(
    schemaPath,
    { root: schemaPath, fileFilter: (f) => f.path.endsWith(schemaExtension) },
  );
