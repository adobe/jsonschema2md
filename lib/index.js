...
const {
  iter, pipe, filter, map, obj, pairs,
} = require('ferrum');
...
function jsonschema2md(schema, {
  ...
  if (normalized.$schema || normalized.$id || normalized.title) {
    normalized = {
      'input.schema.json': normalized,
    };
  }
  ...
  const schemas = pipe(
    ...
    map(([name, jsonData]) => {
      ...
      if (typeof jsonData === 'string') {
        // eslint-disable-next-line import/no-dynamic-require, global-require
        content = require(jsonData);
      }
      return schemaLoader(name, content);
    }),
    traverse,
  );
