...
function gettype(schema) {
  if (typeof schema[keyword`type`] === 'string') {
    return schema[keyword`type`];
  }
  if (Array.isArray(schema[keyword`type`])) {
    return 'multiple';
  }
  if (isabstract(schema)) {
    return gettype(schema.definitions);
  }
  if (schema[keyword`type`] === undefined) {
    return 'undefined';
  }
  if (schema[keyword`oneOf`] || schema[keyword`anyOf`] || schema[keyword`allOf`] || schema[keyword`not`]) {
    return 'merged';
  }
  // console.log('Unknown type', schema[keyword`type`]);
  return 'undefined';
}
