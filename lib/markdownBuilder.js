...
function build({
  ...
  function maketypefact(definition, isarray = '') {
    ...
    if (array && Array.isArray(definition[keyword`items`])) {
      return makearrayfact(definition[keyword`items`], definition.additionalItems);
  ...
  function makeconstraintssection(schema, level = 1) {
    ...
    if (schema[keyword`minContains`] && schema[keyword`contains`]) {
      ...
      constraints.push(paragraph([strong(text(i18n`minimum number of contained items`)), text(': '), text(i18n`this array may not contain more than ${schema[keyword`minContains`]} items that validate against the schema:`),
        link(`${schema[keyword`contains`][s.slug].md}`, i18n`check type definition`, gentitle(schema[keyword`contains`][s.titles], schema[keyword`contains`][keyword`type`]))]));
    }
    if (schema[keyword`maxContains`] && schema[keyword`contains`]) {
      // console.log('maxContains!', schema[s.filename], schema[s.pointer]);
      constraints.push(paragraph([strong(text(i18n`maximum number of contained items`)), text(': '), text(i18n`this array may not contain fewer than ${schema[keyword`maxContains`]} items that validate against the schema:`),
        link(`${schema[keyword`contains`][s.slug].md}`, i18n`check type definition`, gentitle(schema[keyword`contains`][s.titles], schema[keyword`contains`][keyword`type`]))]));
    }
