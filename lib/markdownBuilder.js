 ...
 */
import {
  map, list as flist, flat, filter, size, foldl,
} from 'ferrum';
import {
  root,
  paragraph,
...
import { gentitle } from './formattingTools.js';
...
export default function build({
  ...
  return (schemas) => foldl(schemas, {}, (pv, schema) => {
    ...
    // Calculate depth and determine appropriate base level
    // Schemas that are part of allOf/anyOf/oneOf constructs within properties
    // should use higher header levels to maintain hierarchy
    const pointer = schema[s.pointer];
    
    // Check if this schema is part of an allOf/anyOf/oneOf within a property
    // by looking at the pointer path
    const isCompositionSchema = /\/(allOf|anyOf|oneOf)\/\d+/.test(pointer);
    const propertyDepth = (pointer.match(/\/properties\//g) || []).length;
    
    // Base level calculation:
    // - Root schemas and direct properties: level 1
    // - Schemas within allOf/anyOf/oneOf of properties: use property depth to offset
    // This maintains proper hierarchy when schemas are viewed in context
    let baseLevel = 1;
    if (isCompositionSchema && propertyDepth > 0) {
      // For composition schemas (allOf/anyOf/oneOf), offset the base level
      // based on how deep they are in the property hierarchy
      baseLevel = propertyDepth;
    }
