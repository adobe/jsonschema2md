  ...
  function maketypefact(definition, isarray = '') {
    ...
    const typefact = (() => {
      if (nulltype) {
        ...
        return [text(isarray ? i18n`an array of the following:` : i18n`any of the folllowing: `), ...flist(flat(realtypes.map((type, index) => [inlineCode(type || i18n`not defined`), text(index === realtypes.length - 1 ? '' : i18n` or `)])))];
      } else if (merged) {
