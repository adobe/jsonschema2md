...
function generateNewSchemaFiles(filename, schema) {
    return writeFile(path.join(path.join(outDir,"_new"), path.dirname(filename.substr(schemaPath.length))), path.basename(filename), JSON.stringify(schema, null, 4));
...
var GenerateForAllFiles = function (dirname) {
  ...
  return new Promise((resolve,reject) => {
    ...
    .on('data',(entry) => {
      ...
      ajv.addSchema(require(entry.fullPath),entry.path);
    })
    ...
    return Promise.map(schemaFiles,function(file){
      ...
      .then((processedSchemas) => {
        if(argv.w)
          return Promise.all([generateMarkdown(file,processedSchemas.mSchema),generateNewSchemaFiles(file,processedSchemas.wSchema)]);
        else
