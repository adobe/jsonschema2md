  ...
  function toJsonLd(next) {
    ...
    schema.toJsonLD(function (err, doc, context) {
      if (err) {
        ...
        writeFile(path.join(outDir, 'json'), baseName + '.jsonld', JSON.stringify(doc, null, 2), next);
        writeFile(path.join(outDir, 'json'), 'context.jsonld', JSON.stringify(context, null, 2), next);
      }
    });
  }
