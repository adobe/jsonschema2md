...
Schema.load = function(schemaFile, baseURL, cb){
  ...
  function resolveReferences(schemaFile,callback){
    jsonSchemaRefParser.dereference(JSON.parse(schemaFile),{ resolve: {external:false}},function(rerr, rschema){
      if(rerr)
        callback(rerr,null);
      else{
        processSchema(baseURL,rschema,false, function(err, schema) {
            callback(err,schema);
        });
      }
    });
