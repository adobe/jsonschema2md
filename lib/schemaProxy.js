...
const handler = ({
  root = '', inputpath = '.', filename = '.', schemas, parent = null, slugger,
}) => {
  const meta = {};
  ...
  meta[symbols.inputpath] = () => inputpath;
  ...
  return {
    ...
    get: (target, prop, receiver) => {
      ...
      if (retval === undefined && prop === keyword`examples` && !receiver[symbols.parent]) {
        return loadExamples(receiver[symbols.inputpath], 1);
      }
      if (typeof retval === 'object' && retval !== null) {
        if (retval[keyword`$ref`]) {
          ...
          if (schemas.known[basedoc]) {
            ...
            const basepath = path.dirname(meta[symbols.inputpath]());
            let reldoc = uri;
...
module.exports.loader = () => {
  ...
  return (name, schema) => {
    ...
    const inputpath = name === filename ? undefined : name;
    const proxied = new Proxy(schema, handler({
      filename, inputpath, schemas, slugger,
    }));
    ...
    schemas.files[inputpath || filename] = proxied;
